...
export default function HomePage() {
  return (
    <div className="flex flex-col min-h-dvh bg-white dark:bg-gray-900 text-foreground">
      <Header />
      <main className="flex-1 pt-20 bg-white dark:bg-gray-900">
        {/* Hero Section */}
        <section className="relative w-full py-20 md:py-32 lg:py-40 bg-white dark:bg-gray-900 overflow-hidden" dir="rtl">
          {/* Decorative shapes */}
          <div className="absolute top-20 right-10 w-72 h-72 bg-cyan-400/15 dark:bg-cyan-400/15 rounded-full blur-3xl" />
          <div className="absolute bottom-10 left-20 w-96 h-96 bg-orange-400/10 dark:bg-orange-400/10 rounded-full blur-3xl" />

          <div className="container mx-auto px-6 md:px-8 lg:px-12 relative z-10">
            <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 border border-cyan-600/40 dark:border-cyan-400/30 text-cyan-700 dark:text-cyan-400 text-sm font-medium mb-6">
              ...
              <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl text-navy-900 dark:text-white mb-6 text-balance leading-tight">
        ...
        <section className="relative -mt-12 z-20 px-6 md:px-8 lg:px-12" dir="rtl">
          <div className="container mx-auto">
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 md:p-10 shadow-lg shadow-black/10 dark:shadow-black/40 border border-gray-200 dark:border-gray-700">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                {stats.map((stat, i) => (
                  <div key={i} className="text-center">
                    <div className="text-3xl md:text-4xl font-extrabold text-cyan-600 dark:text-cyan-400 mb-1">{stat.value}</div>
        ...
        {/* Core Values Section */}
        <section className="w-full py-20 md:py-28 bg-gray-50 dark:bg-gray-900" dir="rtl">
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {values.map((item, i) => (
                <div key={i} className="text-center group">
                  <div className="w-16 h-16 rounded-2xl bg-cyan-100 dark:bg-cyan-500/20 flex items-center justify-center mx-auto mb-5 group-hover:bg-cyan-200 dark:group-hover:bg-cyan-500/30 transition-colors duration-300">
                    <item.icon className="h-8 w-8 text-cyan-700 dark:text-cyan-400" />
                  </div>
        ...
        <section className="w-full py-20 md:py-28 bg-white dark:bg-gray-800" dir="rtl">
          <div className="container mx-auto px-6 md:px-8 lg:px-12">
            <div className="text-center mb-14">
              <h2 className="text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl text-navy-900 dark:text-white mb-4 text-balance">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {services.map((service) => (
                <Link
                  key={service.href}
                  href={service.href}
                  className="group block rounded-2xl bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 p-6 hover:bg-gray-100 dark:hover:bg-gray-600 hover:border-cyan-600 dark:hover:border-cyan-400/50 transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="w-12 h-12 rounded-xl bg-cyan-100 dark:bg-cyan-500/25 flex items-center justify-center mb-4 group-hover:bg-cyan-200 dark:group-hover:bg-cyan-500/35 transition-colors">
                    <service.icon className="h-6 w-6 text-cyan-700 dark:text-cyan-400" />
                  </div>
                  <h3 className="text-lg font-bold text-navy-900 dark:text-white mb-2">{service.title}</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 leading-relaxed">{service.desc}</p>
                  <ul className="space-y-2">
                    {service.features.map((f, j) => (
                      <li key={j} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        <CheckCircle className="h-3.5 w-3.5 text-cyan-700 dark:text-cyan-400 shrink-0" />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <div className="mt-5 flex items-center gap-2 text-cyan-700 dark:text-cyan-400 text-sm font-medium group-hover:gap-3 transition-all">
                    اقرأ المزيد
          ...
          <div className="container mx-auto px-6 md:px-8 lg:px-12">
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {excellence.map((item, i) => (
                <div key={i} className="rounded-2xl p-8 text-center hover:-translate-y-1 transition-all duration-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg hover:shadow-orange-400/10 dark:hover:shadow-orange-400/10">
                  <div className="w-14 h-14 rounded-xl bg-orange-100 dark:bg-orange-500/25 flex items-center justify-center mx-auto mb-5">
